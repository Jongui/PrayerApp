  ...
  _uploadImage() async {
    if (_newImage != null) {
      ...
      Future fut = PrayFirebase().uploadPrayAlbumPicture(
          this.widget.pray.idPray, _newImage, _newImageDescription,);
      fut.then((mapValues) {
        ...
        setState(() {
          ...
          _widgets.sort((view1, view2) =>
              view2.fileName.compareTo(view1.fileName)
          );
          List<Widget> _newWidgets = [];
          _widgets.forEach((widget) async{
            _newWidgets.add(AlbumPictureCardView(pictureUrl: widget.pictureUrl, fileName: widget.fileName, pray: widget.pray));
          });
          _view = ListView(
            shrinkWrap: true,
